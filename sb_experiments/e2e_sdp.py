...
if __name__ == "__main__":
    ...
    # Calibrate the model
    sdp_model.calibrate(X=X_cal, y=y_cal)
    sdp_model.metrics["hsic_cal_full"] = sdp_model.compute_HSIC(X=X_cal, y=y_cal, extra=False)

    # Predict
    mean_prediction, variance_prediction = sdp_model.predict(X=X_test, y=y_test)
    sdp_model.metrics["hsic_test_full"] = sdp_model.compute_HSIC(X=X_test, y=y_test, extra=False)
    ...
    plot_FILE_PATH_OUT_S3 = FOLDER_PATH_OUT_S3 + "figure.pdf"

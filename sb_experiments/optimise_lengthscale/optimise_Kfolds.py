...
def restart_optimise(num_restarts, settings):
    ...
    for restart in range(num_restarts):
        ...
        objective_function_one_param = partial(
            objective_function, settings=settings, log=log
        )
    ...
    for k, (train_index, test_index) in enumerate(kf.split(X_train)):
        ...
        sdp_model = ub.UniversalFunctionAndBandsRegressor(
            ...
            variance_kernel=kernels.Matern(
                length_scale=theta_v, length_scale_bounds=(1e-5, 1e5), nu=2.5
            ),
            lambda2=lambda2,
            delta=delta,
...
if __name__ == "__main__":
    ...
    start = time.time()
    best_theta, best_hsic = restart_optimise(

"""
First training a Gaussian Process with three parameters.
We use the posterior kernel to compute s and in the SDP problem.
...
def compute(X_train, y_train, theta_v, X_cal, y_cal, X_test, y_test, save):
    ...
    sdp_model = ub.UniversalFunctionAndBandsRegressor(
        mean_kernel=Matern(
            length_scale=theta_m["posterior_lengthscale"],
            sigma=theta_m["posterior_variance"],
            nu=2.5,
            nugget=theta_m["posterior_nugget"],
        ),
    ...
    print(f"Start training: {theta_v}")
    # Actually solve the SDP.
    sdp_model.train(X=X_train, y=y_train)
    print(f"Training took: {sdp_model.solver_time}")
    ...
    print(f"Start calibration: {theta_v}")
    # Calibrate the model.
    ...
    print(f"Start inference: {theta_v}")
    # Infer.
...
if __name__ == "__main__":
    ...
    for theta_v in all_theta_v:
        BUCKET_OUT = f"luisito/these/sb_experiments/which_kernel/results/three_parameters/lengthscale_{theta_v}/"
